...
def generatePrimes(n):
    ...
    sieve = eratosthenes(n + 1)
    return [i for i in xrange(3, n + 1) if sieve[i]]

def primeSummations(n):
    primes = generatePrimes(n - 1)
    poly = buildPolynomial(2, n)
    for prime in primes:
        poly *= buildPolynomial(prime, n)
    return poly.coeff(n)
    
def main():
    primes = generatePrimes(74)
    poly = buildPolynomial(2, 75)
    for prime in primes:
        poly *= buildPolynomial(prime, 75)
    for i in xrange(76):
        if poly.coeff(i) > 5000:
            print i
            break
